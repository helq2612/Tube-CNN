...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        '''
        inds_inside = self.inds_inside.cuda()
        anchors = self._anchors.cuda()
        ...
        # labels.dim()=2
        labels = _unmap(labels, self.num_anchors_total, inds_inside, batch_size, fill=-1)
        bbox_targets = _unmap(bbox_targets, self.num_anchors_total, inds_inside, batch_size, fill=0)
        bbox_inside_weights = _unmap(bbox_inside_weights, self.num_anchors_total, inds_inside, batch_size, fill=0)
        bbox_outside_weights = _unmap(bbox_outside_weights, self.num_anchors_total, inds_inside, batch_size, fill=0)
