...
def _smooth_l1_loss1(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    print("inside", bbox_pred, bbox_targets)
    sigma_2 = sigma ** 2
    box_diff = bbox_pred - bbox_targets
    print("inside1", box_diff)
    in_box_diff = bbox_inside_weights * box_diff
    print("inside2", in_box_diff)
    abs_in_box_diff = torch.abs(in_box_diff)
    print("inside3", abs_in_box_diff)
    smoothL1_sign = (abs_in_box_diff < 1. / sigma_2).detach().float()
    print("inside4", smoothL1_sign)
    in_loss_box = torch.pow(in_box_diff, 2) * (sigma_2 / 2.) * smoothL1_sign \
                  + (abs_in_box_diff - (0.5 / sigma_2)) * (1. - smoothL1_sign)
    print("inside5", in_loss_box)
    out_loss_box = bbox_outside_weights * in_loss_box
    print("inside6", out_loss_box)
    loss_box = out_loss_box
    print("inside7")
    for i in sorted(dim, reverse=True):
        loss_box = loss_box.sum(i)
