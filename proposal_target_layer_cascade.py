...
class _ProposalTargetLayer(nn.Module):
    ...
    def forward(self, all_rois, gt_boxes):
        ...
        # heli add: since the gt_boxes size is 1x8x4, but here need 1x8x5, so modify it
        gt_boxes_append = gt_boxes.new(gt_boxes.size()).zero_()
#         gt_size = (gt_boxes.size(0), gt_boxes.size(1), gt_boxes.size(2) + 1)
#         gt_boxes_append = torch.zeros(gt_size)
        gt_boxes_append[:,:,1:5] = gt_boxes[:,:,:4]
        
        # Include ground-truth boxes in the set of candidate rois
        # here all_rois is 'torch.cuda.FloatTensor', and gt_boxes_append is 'torch.FloatTensor'
        gt_boxes_append = gt_boxes_append.cuda()
        all_rois = torch.cat([all_rois, gt_boxes_append], 1)        
        
        num_images = 1
        rois_per_image = int(cfg.TRAIN.BATCH_SIZE / num_images)  # 128/1 =1
    ...
    def _sample_rois_pytorch(self, all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
        ...
        labels_batch = labels.new(batch_size, rois_per_image).zero_()              ## size of: 4x128
